...
$$Y_{ij}|\beta_{0},\beta_1,\sigma,\tau, u, x \sim N(\beta_{0} + u_j + \beta_1 x_{ij},\sigma^2)$$
...
Sometimes, analysts derive a Bayesian "p-value" which is the posterior probability of the null hypothesis. If this probability is small, then the null hypothesis can be rejected. Note that this is a pragmatic approach adopting frequentist terminology and not part of Bayesian theory. 

For example, we specify the following hypotheses: 

$H_0: \beta \leq 0$ against $H_1: \beta > 0$ 

The Bayesian p-value is then $p=\int_{-\infty}^0 \beta f(\beta|data)d\beta$ where $f(\beta|data)$ denotes the posterior density. When we have a sample from the posterior, $b_1,\ldots, b_n$, we can approximate this probability as the proportion of samples that comply with $H_0$, i.e. $\hat{p} = \frac{1}{n}\sum_{i=1}^{n} I_{b_i\leq 0}$

### Deviance Information Criterion

Bayesian testing can be done by comparing alternative models. A suggested approach is to comparing deviances based on the posterior.  

The deviance information criterion (DIC) can be defined as

$$DIC = D(\bar{\theta})+2p_D$$

where $\bar{\theta}$ is the posterior mean and $p_D$ the effective number of parameters of the model. Most MCMC samplers offers ways to derive the DIC. 
...
Another approach is to use **Bayes factor** to compare two models, $M_0$ and $M_1$:   
...
A Bayes factor much greater than 1 gives high evidence to the model in the nominator, $M_1$. There are scales to clasify the strengths of evidence for one model compared to another. [Example of scale](https://www.statisticshowto.com/bayes-factor-definition/)
